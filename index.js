...
async function postReviewComments(octokit, comments, repo, prNumber) {
    ...
    try {
        ...
        const formattedComments = comments
            .map(comment => {
                const file = pullRequestDiff.find(f => f.filename === comment.filename);
                if (!file) {
                    console.error(`File ${comment.filename} not found in PR.`);
                    return null;
                }

                const position = getDiffPosition(file.patch, comment.line);
                if (position === null) {
                    console.error(`Could not determine diff position for ${comment.filename}:${comment.line}`);
                    return null;
                }
                ...
                return {
                    ...
                    position,
                    body: `### 💡 Code Review for \`${comment.filename}\`\n\n` +
