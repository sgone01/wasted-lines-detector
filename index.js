...
❌ AI Suggestion failed.`;
    ...
    // Extracting parts of the suggestion
    ...
    let optimizedCode = "No code suggestion provided.";
    let complexityAnalysis = "No complexity analysis provided.";

    if (optimizedCodeIndex !== -1) {
        optimizedCode = suggestionLines.slice(optimizedCodeIndex + 1, complexityIndex !== -1 ? complexityIndex : undefined).join(" \n");
    }
    
    if (complexityIndex !== -1) {
        complexityAnalysis = suggestionLines.slice(complexityIndex).join(" \n");
    }

    // Ensure a valid optimized code suggestion
    if (!optimizedCode || optimizedCode.toLowerCase().includes("no code suggestion")) {
        optimizedCode = "// ✅ Remove unused variables to improve readability and maintainability.";
    }
