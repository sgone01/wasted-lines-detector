...
function formatGitHubReviewComment(filename, originalContent, suggestion) {
    if (!suggestion || suggestion.trim() === "") {
        ...
        return null; // Return null to avoid posting empty comments
    }

    const suggestionLines = suggestion.split("\n").map(line => line.trim()).filter(line => line !== "");
    
    // Extract optimized code (first block) and complexity (remaining)
    const optimizedCode = suggestionLines[0] || "No code suggestion provided.";
    const complexityAnalysis = suggestionLines.slice(1).join(" ") || "No complexity analysis provided.";

    return {
        path: filename,
        body: `### 💡 Code Review & Optimization for \`${filename}\`\n\n` +
